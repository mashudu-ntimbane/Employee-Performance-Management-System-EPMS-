...
<style>
 ...
 /* App Layout - header touches nav seamlessly */
        ...
        /* Sent Messages List Modal Specific Styles */
        .sent-message-item {
            background: var(--bg-card);
            border-radius: 14px;
            padding: 18px 22px;
        ...
        .sent-message-item .message-receiver-name {
            font-size: 1rem;
            color: var(--primary-mid);
            margin-bottom: 4px;
            display: flex;
        ...
        .sent-message-item .message-heading-text {
            font-size: 0.95rem;
            color: var(--text-main);
            white-space: nowrap;
            overflow: hidden;
        ...
        .sent-detail-modal .modal-header {
            background: linear-gradient(90deg, #0c1e3d 0%, #1e3a5f 40%, #4f46e5 100%);
        }
...
<div class="app-layout">
    ...
    <aside class="side-nav">
        ...
        <a href="messagesMa.php" class="nav-link-item active">
    ...
    <main class="main-content">
        <div class="page-header">
            ...
            <div style="display: flex; gap: 12px;">
                <button class="btn btn-gradient-indigo" onclick="openSentMessagesModal()">
                    <i class="fas fa-paper-plane" style="margin-right: 6px;"></i>Sent Messages
                </button>
                <button class="btn btn-gradient-teal" onclick="document.location='send_messageMa.php'">
        ...
        <!-- Sent Messages List Modal -->
        <div class="modal fade message-modal" id="sentMessagesModal" tabindex="-1" aria-labelledby="sentMessagesModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body" id="sentMessagesModalBody">
                        <div class="text-center py-5">
                            <div class="spinner-border" style="color: var(--accent-indigo);" role="status">
                                <span class="visually-hidden">Loading...</span>
                            </div>
        ...
        <div class="modal fade message-modal sent-detail-modal" id="sentMessageDetailModal" tabindex="-1" aria-labelledby="sentMessageDetailModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="sentMessageDetailModalLabel">
                            <i class="fas fa-envelope-open-text"></i>
                            <span>Sent Message Details</span>
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" id="sentMessageDetailModalBody">
                        <div class="text-center py-5">
                            <div class="spinner-border" style="color: var(--
accent-indigo);" role="status">
                                <span class="visually-hidden">Loading...</span>
                            </div>
                            <p class="mt-3 text-muted">Loading message details...</p>
                        </div>
                </div>
        </div>
    </main>
...
<script>
    ...
    const sentMessagesModal = new bootstrap.Modal(document.getElementById('sentMessagesModal'));
    const sentMessagesModalBody = document.getElementById('sentMessagesModalBody');
    const sentMessageDetailModal = new bootstrap.Modal(document.getElementById('sentMessageDetailModal'));
    const sentMessageDetailModalBody = document.getElementById('sentMessageDetailModalBody');
    ...
    function openSentMessagesModal() {
        sentMessagesModalBody.innerHTML = `
            <div class="text-center py-5">
                <div class="spinner-border" style="color: var(--accent-indigo);" role="status">
                    <span class="visually-hidden">Loading...</span>
                </div>
        ...
        `;
        sentMessagesModal.show();

        fetch('get_sent_messages.php')
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    sentMessagesModalBody.innerHTML = `<div class="alert alert-danger">${data.error}</div>`;
                    return;
                }
                ...
                messages.forEach(msg => {
                    ...
                    if (timeDifference < 86400) {
                        formattedDate = "Today, " + messageTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
                    } else if (timeDifference < 172800) {
                        formattedDate = "Yesterday, " + messageTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
                    } else {
                        formattedDate = messageTime.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                    }

                    html += `
                        <div class="sent-message-item" onclick="openSentMessageDetail(${msg.id})">
                            <div class="message-content">
                                <div class="message-receiver-name">
                                    <i class="fas fa-user-circle" style="color: var(--accent-indigo); font-size: 1.1rem;"></i>
                                    To: ${escapeHtml(msg.receiver_name)} ${escapeHtml(msg.empLname)}
                                </div>
                                <div class="message-heading-text">${escapeHtml(msg.heading)}</div>
                            <div class="message-meta">
                                <div class="message-timestamp"><i class="far fa-clock" style="margin-right: 4px; color: #94a3b8;"></i>${formattedDate}</div>
                                <div class="message-actions">
                                    ${msg.replied ? '<span class="message-badge badge-replied"><i class="fas fa-reply"></i> Replied</span>' : ''}
                                </div>
                        </div>
                    `;
                });
                ...
                html += '</div>';
                sentMessagesModalBody.innerHTML = html;
                sentMessagesModalBody.scrollTop = 0;
            })
            .catch(err => {
                console.error(err);
                sentMessagesModalBody.innerHTML = `<div class="alert alert-danger">Failed to load sent messages. Please try again.</div>`;
            });
    }

    function openSentMessageDetail(messageId) {
        sentMessagesModal.hide();

        sentMessageDetailModalBody.innerHTML = `
            <div class="text-center py-5">
                <div class="spinner-border" style="color: var(--accent-indigo);" role="status">
                    <span class="visually-hidden">Loading...</span>
                </div>
        ...
        `;
        sentMessageDetailModal.show();

        fetch('get_message.php?id=' + encodeURIComponent(messageId))
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    sentMessageDetailModalBody.innerHTML = `<div class="alert alert-danger">${data.error}</div>`;
                    return;
                }
                ...
                html += `
                    <div class="modal-sender-card">
                        <div class="modal-sender-avatar" style="background: linear-gradient(135deg, #4f46e5 0%, #4338ca 100%);">
                            <i class="fas fa-user"></i>
                        </div>
                        <div class="modal-sender-info">
                            <div class="modal-sender-name">To: ${escapeHtml(msg.sender)} ${escapeHtml(msg.empLname)}</div>
                            <div class="modal-message-date">
                                <i class="far fa-clock"></i> ${formatDate(msg.timestamp)}
                            </div>
                    </div>
                ...
                html += `
                    <div class="modal-replies-section">
                        <div class="modal-replies-title">
                            <i class="fas fa-reply"></i> Replies Received
                        </div>
                `;
                ...
                if (replies.length > 0) {
                    ...
                    html += `<div class="modal-empty-replies">No replies yet.</div>`;
                }

                html += `</div>`;

                html += `
                    <button type="button" class="btn btn-gradient-slate" onclick="backToSentMessages()">
                        <i class="fas fa-arrow-left" style="margin-right: 6px;"></i>Back to Sent Messages
                    </button>
                `;

                sentMessageDetailModalBody.innerHTML = html;
                sentMessageDetailModalBody.scrollTop = 0;
            })
            .catch(err => {
                console.error(err);
                sentMessageDetailModalBody.innerHTML = `<div class="alert alert-danger">Failed to load message details. Please try again.</div>`;
            });
    }

    function backToSentMessages() {
        sentMessageDetailModal.hide();
        setTimeout(() => {
            sentMessagesModal.show();
        }, 300);
    }
