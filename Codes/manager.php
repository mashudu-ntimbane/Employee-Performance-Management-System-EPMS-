...
$approved_leave_result = $conn->query("SELECT COUNT(*) as cnt FROM leave_requests WHERE status = 'approved'");
$approved_leave_count = $approved_leave_result ? (int)$approved_leave_result->fetch_assoc()['cnt'] : 0;
// Fetch rejected leave count
$rejected_leave_result = $conn->query("SELECT COUNT(*) as cnt FROM leave_requests WHERE status = 'rejected'");
$rejected_leave_count = $rejected_leave_result ? (int)$rejected_leave_result->fetch_assoc()['cnt'] : 0;
// Fetch currently on leave count (approved and today is between start_date and end_date)
$today = date("Y-m-d");
$on_leave_result = $conn->query("SELECT COUNT(*) as cnt FROM leave_requests WHERE status = 'approved' AND start_date <= '$today' AND end_date >= '$today'");
$on_leave_count = $on_leave_result ? (int)$on_leave_result->fetch_assoc()['cnt'] : 0;
...
// ---------- NEW DASHBOARD DATA QUERIES ----------

// Feedback counts
$feedbackPendingCount = 0;
$feedbackSentCount = 0;
$fbResult = $conn->query("SELECT feedback_sent, COUNT(*) as cnt FROM issue_ratings GROUP BY feedback_sent");
if ($fbResult) {
    while ($fbRow = $fbResult->fetch_assoc()) {
        if ($fbRow['feedback_sent'] == 0) {
            $feedbackPendingCount = $fbRow['cnt'];
        } else {
            $feedbackSentCount = $fbRow['cnt'];
        }
    }
}

// Task tracking progress counts
$taskCounts = [];
$taskResult = $conn->query("SELECT status, COUNT(*) as cnt FROM issues GROUP BY status");
if ($taskResult) {
    while ($tRow = $taskResult->fetch_assoc()) {
        $taskCounts[$tRow['status']] = $tRow['cnt'];
    }
}
$totalIssues = array_sum($taskCounts);
$fixedIssues = $taskCounts['fixed'] ?? 0;
$pendingIssues = $taskCounts['Pending'] ?? 0;

// Issue categories for graph
$issueCategories = [];
$catResult = $conn->query("SELECT issue_category, COUNT(*) as cnt FROM issues GROUP BY issue_category ORDER BY cnt DESC");
if ($catResult) {
    while ($cRow = $catResult->fetch_assoc()) {
        $issueCategories[] = $cRow;
    }
}

// Clocked in / out counts for today
$clockedInCount = 0;
$clockedOutCount = 0;
$ciResult = $conn->query("SELECT COUNT(DISTINCT empID) as cnt FROM clock_in_records WHERE DATE(clock_in_time) = CURDATE() AND clock_out_time IS NULL");
if ($ciResult) { $clockedInCount = (int)$ciResult->fetch_assoc()['cnt']; }
$coResult = $conn->query("SELECT COUNT(DISTINCT empID) as cnt FROM clock_in_records WHERE DATE(clock_in_time) = CURDATE() AND clock_out_time IS NOT NULL");
if ($coResult) { $clockedOutCount = (int)$coResult->fetch_assoc()['cnt']; }

// Employee table data: clock status, work fixed, recent rating, profile photo
$empQuery = "
    SELECT 
        e.empID,
        e.empFname,
        e.empLname,
        e.empRole,
        (SELECT COUNT(*) FROM issues WHERE empID = e.empID AND status = 'fixed') as work_fixed,
        (SELECT graphic_rating FROM issue_ratings r2 JOIN issues i2 ON i2.issueID = r2.issueID WHERE i2.empID = e.empID ORDER BY r2.rating_date DESC LIMIT 1) as recent_rating,
        c.clock_in_time,
        c.clock_out_time
    FROM emplooyedetails e
    LEFT JOIN (
        SELECT empID, clock_in_time, clock_out_time
        FROM clock_in_records
        WHERE DATE(clock_in_time) = CURDATE()
        AND id IN (
            SELECT MAX(id) FROM clock_in_records WHERE DATE(clock_in_time) = CURDATE() GROUP BY empID
        )
...
function getProfilePhoto($empID) {
    $uploadDir = 'uploads/';
    foreach (['jpg', 'jpeg', 'png', 'gif'] as $ext) {
        $file = $uploadDir . 'profile_' . $empID . '.' . $ext;
        if (file_exists($file)) {
...
<div class="app-layout">
    ...
    <aside class="side-nav">
        <a href="manager.php" class="nav-link-item active">
    ...
    <main class="main-content">
        <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4">
                ...
                <div class="dashboard-card h-100 <?= $new_leave_count > 0 ? 'border-accent-gold' : 'border-accent-teal' ?>">
                    ...
                    <div class="card-body-custom">
                        <?php if ($new_leave_count > 0): ?>
                            ...
                            <a href="Leaves.php" class="btn btn-gradient-gold">View Details</a>
                        <?php else: ?>
                ...
                <div class="dashboard-card h-100 border-accent-indigo">
                    ...
                    <div class="card-body-custom">
                        <div class="d-flex justify-content-center gap-3 mb-3 flex-wrap">
                            <div>
                                <div style="font-size: 1.4rem; font-weight: 800; color: #f59e0b;"><?= $new_leave_count; ?></div>
                                <div class="text-muted" style="font-size: 0.75rem; font-weight: 600;">Pending</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.4rem; font-weight: 800; color: #0d9488;"><?= $approved_leave_count; ?></div>
                                <div class="text-muted" style="font-size: 0.75rem; font-weight: 600;">Approved</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.4rem; font-weight: 800; color: #e11d48;"><?= $rejected_leave_count; ?></div>
                                <div class="text-muted" style="font-size: 0.75rem; font-weight: 600;">Rejected</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.4rem; font-weight: 800; color: #4f46e5;"><?= $on_leave_count; ?></div>
                                <div class="text-muted" style="font-size: 0.75rem; font-weight: 600;">On Leave</div>
                            </div>
                        </div>
                        <a href="Leaves.php" class="btn btn-gradient-indigo">Manage Leaves</a>
                    </div>
                </div>
            </div>
                ...
                <div class="dashboard-card h-100 <?= $unreadCount > 0 ? 'border-accent-gold' : 'border-accent-teal' ?>">
                    ...
                    <div class="card-body-custom">
                        <?php if ($unreadCount > 0): ?>
                            ...
                            <a href="messagesMa.php" class="btn btn-gradient-gold">Read Messages</a>
                        <?php else: ?>
                ...
                <div class="dashboard-card h-100 <?= $recentIssue ? 'border-accent-slate' : 'border-accent-teal' ?>">
                    ...
                    <div class="card-body-custom">
                        <?php if ($recentIssue): ?>
                            ...
                            <a href="issueMa.php" class="btn btn-gradient-slate mt-2">View Issues</a>
                        <?php else: ?>
            ...
            <!-- Feedback Card -->
            <div class="col">
                <div class="dashboard-card h-100 <?= $feedbackPendingCount > 0 ? 'border-accent-rose' : 'border-accent-teal' ?>">
                    <div class="card-header-custom">
                        <i class="fas fa-comment-dots" style="color: <?= $feedbackPendingCount > 0 ? '#e11d48' : '#0d9488' ?>"></i>
                        <span>Feedback</span>
                    </div>
                    <div class="card-body-custom">
                        <div class="d-flex justify-content-center gap-4 mb-3">
                            <div>
                                <div style="font-size: 1.6rem; font-weight: 800; color: #e11d48;"><?= $feedbackPendingCount; ?></div>
                                <div class="text-muted" style="font-size: 0.8rem; font-weight: 600;">To Send</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.6rem; font-weight: 800; color: #0d9488;"><?= $feedbackSentCount; ?></div>
                                <div class="text-muted" style="font-size: 0.8rem; font-weight: 600;">Sent</div>
                            </div>
                        </div>
                        <?php if ($feedbackPendingCount > 0): ?>
                            <p class="card-text text-muted mb-3"><?= $feedbackPendingCount; ?> feedback(s) pending your review.</p>
                            <a href="send_feedback_Ma.php" class="btn btn-gradient-rose" style="background: linear-gradient(90deg, #e11d48 0%, #be123c 100%); border: none; color: white; border-radius: 10px; padding: 8px 24px; font-weight: 600;">Send Feedback</a>
                        <?php else: ?>
                            <p class="card-text text-muted mb-3">All feedbacks have been sent.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col">
                <div class="dashboard-card h-100 border-accent-indigo">
                    ...
                    <div class="card-body-custom">
                        <div class="d-flex justify-content-center gap-4 mb-3">
                            <div>
                                <div style="font-size: 1.6rem; font-weight: 800; color: #4f46e5;"><?= $totalIssues; ?></div>
                                <div class="text-muted" style="font-size: 0.8rem; font-weight: 600;">Total</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.6rem; font-weight: 800; color: #0d9488;"><?= $fixedIssues; ?></div>
                                <div class="text-muted" style="font-size: 0.8rem; font-weight: 600;">Fixed</div>
                            </div>
                            <div style="width: 1px; background: #e2e8f0;"></div>
                            <div>
                                <div style="font-size: 1.6rem; font-weight: 800; color: #f59e0b;"><?= $pendingIssues; ?></div>
                                <div class="text-muted" style="font-size: 0.8rem; font-weight: 600;">Pending</div>
                            </div>
                        </div>
                        <a href="Track_issues.php" class="btn btn-gradient-indigo">Track Tasks</a>
                    </div>
                </div>
            </div>
        ...
        <!-- Employee Attendance & Performance Table -->
        <div class="dashboard-card mt-4">
            <div class="card-header-custom">
                <i class="fas fa-users" style="color: #0d9488"></i>
                <span>Employee Attendance & Performance</span>
            </div>
                    ...
                    <table class="table issues-table mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <?php if ($photo): ?>
                                        <img src="<?= htmlspecialchars($photo) ?>" alt="Profile" class="profile-thumb">
                                    <?php else: ?>
                                ...
                                <td><?= htmlspecialchars($emp['empFname'] . ' ' . $emp['empLname']) ?></td>
                                <td><?= htmlspecialchars($emp['empRole']) ?></td>
                                <td><span class="badge <?= $statusClass ?>" style="font-size: 0.8rem;"><?= $status ?></span></td>
                                <td><?= (int)$emp['work_fixed'] ?></td>
                                <td>
                                    <?php if ($emp['recent_rating']): ?>
        ...
        <div class="dashboard-card mt-4">
            <div class="card-header-custom">
                ...
                <span>Issue Categories Overview</span>
            </div>
            <div class="card-body-custom">
                ...
                <div class="empty-state">
                    ...
                    <h5>No Data Yet</h5>
                    <p>No issues have been reported to generate a category breakdown.</p>
                </div>
                <?php endif; ?>
            </div>
        </div>
...
<script>
    ...
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: <?= json_encode(array_column($issueCategories, 'issue_category')) ?>,
            datasets: [{
                label: 'Issues Reported',
                data: <?= json_encode(array_column($issueCategories, 'cnt')) ?>,
                backgroundColor: [
                    '#4f46e5', '#0d9488', '#f59e0b', '#e11d48', '#475569',
                    '#3b82f6', '#10b981', '#f97316', '#ef4444', '#64748b'
                ],
                borderRadius: 8,
                borderSkipped: false
            }]
